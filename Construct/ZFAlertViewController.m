...
@interface ZFHighLightButton : UIButton

@property (strong, nonatomic) UIColor *highlightedColor;

@end

@implementation ZFHighLightButton
...
@property (copy, nonatomic) void(^actionHandler)(ZFAlertAction *action);
...
+ (instancetype)actionWithTitle:(NSString *)title handler:(void (^)(ZFAlertAction *action))handler {
    ZFAlertAction *instance = [ZFAlertAction new];
...
+ (instancetype)alertControllerWithTitle:(NSString *)title message:(NSString *)message {
    ...
    ZFAlertViewController *instance = [ZFAlertViewController new];
...
- (void)creatAllButtons {
    ...
    for (int i=0; i<self.actions.count; i++) {
        ...
        ZFHighLightButton *btn = [ZFHighLightButton new];
        btn.tag = 10+i;
...
- (void)didClickButton:(UIButton *)sender {
    ZFAlertAction *action = self.actions[sender.tag-10];
